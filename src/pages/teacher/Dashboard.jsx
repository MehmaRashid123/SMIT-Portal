  ...
  return (
    <PortalLayout links={NAV} accentColor={G}>
      <div className="p-4 sm:p-6 max-w-5xl mx-auto">
        <div className="mb-6">
          <h1 className="text-xl sm:text-2xl font-extrabold text-gray-900">Welcome, {user?.name || user?.username} 👋</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          <Link to="/teacher/quiz"
            className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-5 flex items-center gap-4 hover:border-green-400 transition-colors group">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background:'#e8faf4' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={G} strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/>
              </svg>
            </div>
            <div>
              <p className="font-bold text-gray-800 group-hover:text-green-700 text-sm">Create Quiz</p>
              <p className="text-xs text-gray-400">Add questions for students</p>
            </div>
          </Link>
          <Link to="/teacher/assignments"
            className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-5 flex items-center gap-4 hover:border-blue-400 transition-colors group">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background:'#e8f4fc' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#0B73B7" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/>
              </svg>
            </div>
            <div>
              <p className="font-bold text-gray-800 group-hover:text-blue-700 text-sm">Add Assignment</p>
              <p className="text-xs text-gray-400">Create new assignment task</p>
            </div>
          </Link>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[480px]">
              <thead className="bg-gray-50">
                <tr>{['Name','Roll No','Course'].map(h=><th key={h} className="text-left px-5 py-3 text-xs font-semibold text-gray-500">{h}</th>)}</tr>
              </thead>
              <tbody>
                {enrollments.slice(0,5).map(e => (
                  <tr key={e.id} className="border-t border-gray-50 hover:bg-gray-50">
                    <td className="px-5 py-3 font-medium text-gray-800">{e.students?.name || '—'}</td>
                    <td className="px-5 py-3 text-gray-500 text-xs">{e.students?.roll_number || '—'}</td>
                    <td className="px-5 py-3 text-gray-500 max-w-[160px] truncate">{e.courses?.name || '—'}</td>
                  </tr>
                ))}
                {enrollments.length === 0 && (
                  <tr><td colSpan={3} className="px-5 py-8 text-center text-gray-400">No students enrolled yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
