...
export default function Login() {
  ...
  const [form, setForm] = useState({ cnic: '', rollNumber: '', password: '' })

  const handleSubmit = async (e) => {
    e.preventDefault()
    const result = await dispatch(loginStudent({ cnic: form.cnic, rollNumber: form.rollNumber, password: form.password }))
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">Sign in to your SMIT account</p>
        </div>
        ...
        <p className="text-center text-sm text-gray-500 mt-2">
          Admin? <Link to="/admin/login" className="text-green-700 font-medium hover:underline">Admin Login</Link>
        </p>
      </div>
    </div>
  )
